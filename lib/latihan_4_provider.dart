import 'package:flutter/material.dart'; // Import library material.dart untuk membuat UI Flutter.
import 'package:http/http.dart'
    as http; // Import library http.dart dengan alias http untuk melakukan permintaan HTTP.
import 'dart:convert'; // Import library convert.dart untuk mengonversi JSON.
import 'package:provider/provider.dart'; // Import library provider.dart untuk manajemen state aplikasi.
  ...
  factory University.fromJson(Map<String, dynamic> json) {
    return University(
      ...
      website: json['web_pages']
          [0], // Mengambil alamat situs web dari data JSON.
    );
  }
}
...
class UniversitiesList {
  ...
  UniversitiesList(
      {required this.universities}); // Konstruktor untuk membuat objek UniversitiesList.
  ...
  factory UniversitiesList.fromJson(List<dynamic> json) {
    ...
    universities = json
        ...
        .toList(); // Mengisi daftar universitas dengan objek University yang dibuat dari data JSON.
    return UniversitiesList(
        universities: universities); // Mengembalikan objek UniversitiesList.
  }
}
...
class _MyAppState extends State<MyApp> {
  late Future<UniversitiesList>
      futureUniversities; // Future dari daftar universitas.
  final String baseUrl =
      "http://universities.hipolabs.com/search?country="; // URL dasar untuk permintaan HTTP.

  @override
  void initState() {
    super.initState();
    futureUniversities = fetchUniversities(
        context); // Memuat daftar universitas saat aplikasi diinisialisasi.
  }
  ...
  Future<UniversitiesList> fetchUniversities(BuildContext context) async {
    final activityModel = Provider.of<ActivityModel>(context,
        listen: false); // Mendapatkan model Activity dari Provider.
    final response = await http.get(Uri.parse(baseUrl +
        activityModel.selectedCountry)); // Mengirim permintaan GET ke server.

    if (response.statusCode == 200) {
      // Jika permintaan berhasil,
      List jsonResponse = json.decode(response.body); // Mendekode respons JSON.
      return UniversitiesList.fromJson(
          jsonResponse); // Mengembalikan objek UniversitiesList.
    } else {
      // Jika terjadi kesalahan,
      throw Exception(
          'Failed to load universities'); // Melontarkan pengecualian.
    }
  }
              ...
              Consumer<ActivityModel>(
                builder: (context, activityModel, child) {
                  return DropdownButton<String>(
                    value: activityModel
                        .selectedCountry, // Nilai DropdownButton yang dipilih.
                    onChanged: (String? newValue) {
                      activityModel.updateCountry(
                          newValue!); // Memperbarui negara yang dipilih.
                      setState(() {
                        futureUniversities = fetchUniversities(
                            context); // Memuat ulang daftar universitas saat negara berubah.
                      });
                    },
                    items: <String>['Indonesia', 'Malaysia', 'Singapore']
                      ...
                      return DropdownMenuItem<String>(
                        value: value,
                        child: Text(
                            value), // Menampilkan nama negara di dalam DropdownButton.
                      );
                    }).toList(),
                  );
                },
                ...
                child: FutureBuilder<UniversitiesList>(
                  ...
                  builder: (context, snapshot) {
                    if (snapshot.hasData) {
                      ...
                      return Text(
                          "${snapshot.error}"); // Tampilkan pesan kesalahan.
                    }
