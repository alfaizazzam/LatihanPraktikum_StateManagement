  ...
  factory University.fromJson(Map<String, dynamic> json) {
    return University(
      ...
      website: json['web_pages'].isNotEmpty
          ? json['web_pages'][0]
          : '', // Mengambil alamat situs web dari data JSON.
    );
  }
}
  ...
  factory UniversityList.fromJson(List<dynamic> json) {
    return UniversityList(
      universities: json
          ...
          .toList(), // Mengembalikan objek UniversitiesList.
    );
  }
}
        ...
        providers: [
          BlocProvider(
            create: (context) => UniversityBloc()
              ..fetchUniversities(
                  'Indonesia'), // Inisialisasi UniversityBloc dan panggil fetchUniversities dengan negara Indonesia
          ),
        ],
                ...
                return DropdownButton<String>(
                  ...
                  onChanged: (String? newValue) {
                    if (newValue != null) {
                      setState(() {
                        selectedCountry =
                            newValue; // Perbarui nilai negara yang dipilih
                      });
            ...
            child: BlocBuilder<UniversityBloc, UniversityList>(
              builder: (context, state) {
                return ListView.builder(
                  itemCount:
                      state.universities.length, // Jumlah item dalam daftar.
                  itemBuilder: (context, index) {
                    final university = state.universities[index];
                    return ListTile(
                      title:
                          Text(university.name), // Tampilkan nama universitas
                      subtitle: Text(
                          university.website), // Tampilkan website universitas
                    );
                  },
                );
              },
