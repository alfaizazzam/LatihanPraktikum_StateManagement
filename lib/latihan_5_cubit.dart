...
import 'package:http/http.dart'
    as http; // Melakukan import library http.dart dengan alias http
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        // Ubah widget body menjadi Column
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Center(
            // Pusatkan dropdown di dalam Column
              ...
              builder: (context, state) {
                return DropdownButton<String>(
                  ...
                  onChanged: (String? newValue) {
                    if (newValue != null) {
                      countryCubit.updateCountry(
                          newValue); // Memperbarui negara yang dipilih.
                      setState(() {
                        futureUniversities = fetchUniversities(context,
                            newValue); // Memuat ulang daftar universitas saat negara berubah.
                      });
                    }
                  },
          ...
          Expanded(
            // Expanded untuk memastikan FutureBuilder mengisi ruang yang tersedia
            child: FutureBuilder<UniversitiesList>(
              future: futureUniversities,
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  // Jika data telah tersedia ketika terkoneksi
                  ...
                  return ListView.builder(
                    itemCount: snapshot
                        .data!.universities.length, // Jumlah item dalam daftar.
  ...
  Future<UniversitiesList> fetchUniversities(
      BuildContext context, String selectedCountry) async {
    const String baseUrl =
        "http://universities.hipolabs.com/search?country="; // URL dasar untuk API
    final response = await http.get(Uri.parse(baseUrl +
        selectedCountry)); // Melakukan request ke API berdasarkan negara yang dipilih
